...
import { OctagonX, AlertTriangle, ShieldCheck, CheckCircle2, ShieldAlert, Copy, Check, ExternalLink, Globe, AlertCircle, Phone, Lock, Eye, CreditCard, FileText, Sparkles } from 'lucide-react';
    ...
    switch (result.riskLevel) {
      case 'STOP':
        return {
          ...
          icon: <OctagonX className="w-10 h-10 text-red-600 animate-pulse shrink-0" />,
...
Kiểm tra tại Khoan Đã! - Trợ lý phòng chống lừa đảo AI`;
  ...
  return (
    <div className="space-y-5 text-slate-800">
      {/* 100% AI Engine Indicator Status */}
      <div className="p-3 bg-gradient-to-r from-purple-900 via-indigo-900 to-slate-900 text-white rounded-xl shadow-md flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2 font-bold">
          <Sparkles className="w-4 h-4 text-purple-300 animate-spin" />
          <span>Hệ Thống Phân Tích:</span>
          <span className="px-2.5 py-0.5 rounded-full bg-purple-500/30 border border-purple-400/50 text-purple-200 font-extrabold uppercase tracking-wide flex items-center gap-1">
            ✨ 100% Trí Tuệ Nhân Tạo (Gemini Multimodal AI)
          </span>
        </div>
        {result.scamCategory && (
          <div className="px-3 py-1 bg-white/10 border border-white/20 rounded-lg text-purple-100 font-semibold text-[11px]">
            Loại hình: <strong>{result.scamCategory}</strong>
          </div>
        )}
      </div>

      {/* Main High-Visibility Headline Banner */}
      <div className={`p-6 rounded-xl border shadow-sm ${badgeStyle.bg} transition-all`}>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-start gap-4">
            {badgeStyle.icon}
            <div>
              <div className="flex items-center gap-2">
                <span className={`px-2.5 py-0.5 rounded font-black text-xs uppercase tracking-wide ${badgeStyle.tagBg}`}>
                  {result.riskLevel}
                </span>
              ...
              <h2 className={`text-2xl sm:text-3xl font-black mt-1 tracking-tight ${badgeStyle.titleColor}`}>
          ...
          <button
            ...
            className="self-stretch sm:self-auto px-4 py-2 bg-white hover:bg-slate-50 border border-slate-300 rounded-lg text-xs font-bold text-slate-700 transition-colors flex items-center justify-center gap-2 shrink-0 shadow-xs"
          >
            {copied ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-4 h-4" />}
            {copied ? 'Đã sao chép Báo cáo!' : 'Sao chép Cảnh báo này'}
          </button>
        </div>
      </div>
      ...
      {result.aiDetailedReasoning && (
        <div className="p-5 bg-gradient-to-br from-purple-50 via-indigo-50/50 to-slate-50 border border-purple-200/80 rounded-xl shadow-xs space-y-2.5">
          <div className="flex items-center gap-2 font-bold text-xs uppercase tracking-wider text-purple-900 border-b border-purple-100 pb-2">
            <Sparkles className="w-4 h-4 text-purple-600 shrink-0" />
            Đánh Giá Chi Tiết Từ Trí Tuệ Nhân Tạo (Gemini AI Reasoning)
          </div>
      ...
      {result.detectedBrandMismatch && result.mismatchDetails && (
        <div className="p-5 bg-red-50 border-2 border-red-300 rounded-xl shadow-xs space-y-3">
          ...
          <p className="text-xs sm:text-sm text-red-950 leading-relaxed">
            Tin nhắn hoặc nội dung tự xưng là thuộc về <strong>"{result.mismatchDetails.claimedEntity}"</strong>, nhưng liên kết yêu cầu truy cập <strong>"{result.mismatchDetails.providedDomain}"</strong> KHÔNG thuộc danh sách tên miền chính thức đã đăng ký!
          </p>

          <div className="p-3 bg-white border border-red-200 rounded-lg space-y-2 text-xs">
      ...
      {(result.extractedSignals?.hasImageAttached || result.extractedSignals?.imageAnalysisSummary || result.extractedSignals?.ocrTextExtracted) && (
        <div className="bg-gradient-to-r from-blue-50 via-slate-50 to-indigo-50 border border-blue-200 rounded-xl p-5 space-y-3.5 shadow-sm">
          <div className="flex items-center justify-between border-b border-blue-100 pb-2.5">
            <h3 className="text-xs font-bold text-blue-900 uppercase tracking-widest flex items-center gap-2">
              <Eye className="w-4 h-4 text-blue-600 shrink-0" />
              Kết quả Phân tích Hình ảnh AI Multimodal Vision (Trích xuất OCR)
            </h3>
            <span className="px-2 py-0.5 bg-blue-600 text-white font-bold text-[10px] rounded uppercase">
              Gemini Vision AI
            </span>
          </div>

          {result.extractedSignals.imageAnalysisSummary && (
            <p className="text-xs sm:text-sm font-semibold text-slate-800 leading-relaxed bg-white/80 p-3 rounded-lg border border-blue-100">
              {result.extractedSignals.imageAnalysisSummary}
            </p>
          )}

          {result.extractedSignals.ocrTextExtracted && (
            <div className="p-3 bg-white border border-blue-200 rounded-lg text-xs font-mono text-slate-700 whitespace-pre-wrap max-h-40 overflow-y-auto space-y-1">
              <div className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1">
                <FileText className="w-3 h-3 text-blue-500" />
                Văn bản trích xuất được từ ảnh màn hình (OCR):
              </div>
      ...
      {result.extractedSignals?.bankAccountDetails && (result.extractedSignals.bankAccountDetails.accountNumber || result.extractedSignals.bankAccountDetails.bankName) && (
        <div className="p-4 bg-amber-50/80 border border-amber-200 rounded-xl space-y-2 text-xs text-amber-950 shadow-xs">
          <div className="flex items-center gap-2 font-bold text-amber-900 uppercase tracking-wide">
            ...
            Thông tin tài khoản ngân hàng trích xuất được
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 p-3 bg-white border border-amber-200 rounded-lg">
            <div>
        ...
        {result.reasons.length > 0 ? (
          <ul className="space-y-2">
            {result.reasons.map((reason, idx) => (
              <li key={idx} className="flex items-start gap-2.5 text-xs sm:text-sm text-slate-800 bg-slate-50 p-3 rounded-lg border border-slate-200">
                <span className="w-5 h-5 rounded-full bg-red-100 text-red-700 border border-red-200 font-bold text-[11px] flex items-center justify-center shrink-0 mt-0.5">
                  {idx + 1}
                </span>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-3 shadow-sm">
        <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
          ...
          Hành động bạn nên thực hiện ngay lúc này
        </h3>

        <div className="grid grid-cols-1 gap-2.5">
          {result.actionSteps.map((step, idx) => (
            <div key={idx} className="flex items-start gap-3 p-3 bg-slate-50 border border-slate-200 rounded-lg text-xs sm:text-sm text-slate-800">
      ...
      <div className="p-4 bg-white border border-slate-200 rounded-xl space-y-2 text-xs text-slate-700 shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 pb-2">
          ...
          <span className={`px-2 py-0.5 rounded text-[11px] font-bold ${
            ...
            {result.safeBrowsingStatus.checked
              ? result.safeBrowsingStatus.hasMatch ? 'PHÁT HIỆN MÃ ĐỘC/SỐT' : 'KHÔNG CÓ TRONG BLACKLIST'
              : 'TỰ ĐỘNG LƯU THÔNG'}
          </span>
        </div>
      ...
      {/* Disclaimer Notice */}
      <div className="p-4 bg-slate-900 rounded-lg text-[11px] text-slate-200 leading-relaxed flex items-center gap-3">
