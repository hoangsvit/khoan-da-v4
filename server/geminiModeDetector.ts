...
export async function detectConsumerModeWithGemini(
  ...
  const model = String(process.env.GEMINI_ROUTER_MODEL || process.env.GEMINI_MODEL || 'gemini-flash-latest').trim();
  ...
  try {
    ...
    const message = String(cause?.message || cause || 'Gemini route detection failed');
    const isRateLimit = message.includes('429') || message.includes('RESOURCE_EXHAUSTED') || message.includes('Quota exceeded');
    const error: any = new Error(message);
    error.status = isRateLimit ? 429 : 503;
    error.code = isRateLimit ? 'GEMINI_RATE_LIMITED' : 'GEMINI_ROUTING_FAILED';
